...
use Trinity\API\Http;
use Trinity\Struct;

/**
 * Represents the HTTP request class for the API.
 ...
 */
class Request implements Struct\Request
{
  public const SERVER_ADDR = "localhost";
  ...
  public function req(string $type, string $path = null, array $json = [], int $port = self::SERVER_PORT): array
  {
    ...
    $_request = null;
    $_response = null;
    $_json = (!empty($json)) ? http_build_query($json) : null; // # (![]) ? [...] : 0;
